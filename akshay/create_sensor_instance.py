...
def create_instance_DB():
    ...
    for i in sensor_instances:
        ...
        if sensorType in DB["sensorType"]:
            DB["sensorType"][sensorType].append([sensorIP, sensorPort])
        else:
            DB["sensorType"][sensorType] = [[sensorIP, sensorPort]]

        if manufacturer in DB["manufacturer"]:
            DB["manufacturer"][manufacturer].append([sensorIP, sensorPort])
        else:
            DB["manufacturer"][manufacturer] = [[sensorIP, sensorPort]]
        ...
        for key in sensor_instances[i]["attributes"]:
            ...
            if value in DB[key]:
                DB[key][value].append([sensorIP, sensorPort])
            else:
                DB[key][value] = [[sensorIP, sensorPort]]
